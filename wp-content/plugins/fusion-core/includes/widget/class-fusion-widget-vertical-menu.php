...
class Fusion_Widget_Vertical_Menu extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		// Dynamic Styles.
		$style  = '<style>';
		$style .= '#' . $widget_id_escaped . ' ul.menu li a {'; // phpcs:ignore WordPress.Security.EscapeOutput
		...
		if ( 'custom_menu' === $nav_type ) {
			...
			echo '<nav id="' . $widget_id_escaped . '" class="fusion-vertical-menu-widget fusion-menu ' . esc_attr( $instance['behavior'] ) . ' ' . esc_attr( $instance['layout'] ) . ' ' . esc_attr( $widget_border_class ) . '" aria-label="' . esc_attr( $aria_label ) . '">'; // phpcs:ignore WordPress.Security.EscapeOutput
			...
			$html  = '<nav class="fusion-vertical-menu-widget fusion-menu ' . $instance['behavior'] . ' ' . $instance['layout'] . ' ' . $widget_border_class . '" id="' . esc_attr( $args['widget_id'] ) . '">';
			$html .= '<ul class="menu">';
